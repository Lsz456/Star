...
<%
    ...
    Tv_Pending tp = (Tv_Pending) request.getAttribute("tp");
...
<head>
    ...
    <title>星辰·动漫</title>
    <base href="<%=basePath%>">
    <link rel="stylesheet" type="text/css" href="css/stylePlayTV.css">
...
<div class="box">
    ...
    <div class="xqy">
        <!--播放开始-->
        <div class="play">
            <iframe id="video" src="<%=url%><%=tp.getT_p_movieUrls()%>"
        ...
        <div class="intro">
            <div class="switchRoutes">
                <span><a href="teleplay/play.com?play_Id=<%=tv.getT_id()%>&numInt=<%=tp.getT_p_numInt() == 1 ? 1 : tp.getT_p_numInt() - 1%>">< 上一集</a></span>
                <span class="interval">
                    <a href="teleplay/play.com?play_Id=<%=tv.getT_id()%>&numInt=<%=tp.getT_p_numInt().equals(maxNumber) ? tp.getT_p_numInt() : tp.getT_p_numInt() + 1%>">下一集 ></a>
                </span>
                <span id="spacing" class="interval">切换线路</span>
                <ul id="conceal" style="display: none">
                    <li><a href="teleplay/play.com?play_Id=<%=tv.getT_id()%>&numInt=<%=tp.getT_p_numInt()%>">线路1</a></li>
<%--                    <li><a href="cartoon/switchRoutes.com?num_Id=<%=ap.getA_p_id()%>&play_Id=<%=anime.getA_id()%>">线路2</a></li>--%>
                </ul>
            </div>
            <h2><%=tv.getT_title()%></h2>
            <p><%=tv.getT_type()%><span id="vanish" style="cursor: pointer">展开 ﹀</span></p>
            <div class="introduce">
                <p>主演：<%=tv.getT_star()%></p>
                <p>类型：<%=tv.getT_type()%></p>
                <p>简介：<%=tv.getT_description()%>
